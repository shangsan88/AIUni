...
export function CodeReviewPanel({
  ...
  reviews: Array<{ id: string; title: string; review: CodeReviewResult }>;
}) {
  return (
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span
                className={`rounded-full px-3 py-1 text-xs font-semibold ${passed ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}
              >
            ...
            {item.review.missingPoints?.length > 0 ? (
              <div className="mt-3">
                ...
                <ul className="mt-1 list-disc pl-4 text-sm text-muted-foreground">
                  {item.review.missingPoints.map((point, i) => (
                    <li key={i}>{point}</li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Time: {item.review.timeComplexity}
                </p>
                <p className="text-sm text-muted-foreground">
                  Space: {item.review.spaceComplexity}
                </p>
              </div>
            </div>
