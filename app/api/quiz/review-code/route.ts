...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = JSON.parse(jsonMatch[0]) as { score?: unknown; verdict?: unknown };
    ...
    const verdict = normalizedScore >= 5 ? 'pass' : 'fail';
    return apiSuccess({ ...parsed, score: normalizedScore, verdict });
  } catch (error) {
    return apiError(
      'INTERNAL_ERROR',
      500,
